...
The initial FreeToken control was 6.3% below llama.cpp. A historical opt-in run
of the in-tree native HIP Triton router recorded 50.63 TPS while preserving the
same derivation and 54-token prompt, 0.7% above the matching llama.cpp control.
That historical result does not requalify the router after this rebase: ROCm
continues to use the PyTorch reference router by default. Set
`FREETOKEN_ROCM_TRITON_ROUTER=1` only for a bounded validation run.
