...
def test_q4_k_reference_decoder_handles_scale_minimum_and_nibble_order():
    ...
    # Group zero stores its six-bit scale in scales[0] and its minimum in scales[4].
    raw[0, 4] = 3
    raw[0, 8] = 4
    raw[0, 16:32] = 0xF1  # low nibble 1, high nibble 15 for the first 32-value group.

    decoded = dequant_q4_k(raw, torch.float32)
    # group 0: 2 * 3 * q - 0.5 * 4. The first 16 values are low nibbles, then high.
    assert decoded[0].item() == 4.0
    assert decoded[15].item() == 4.0
    assert decoded[16].item() == 88.0
