...
class VocabParallelEmbedding(BaseOP):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.weight.device.type == "cpu":
            # Experimental CPU embedding path.
            # Input IDs are tiny; only the resulting hidden states are copied
            # back to the GPU.
            indices_cpu = x.detach().cpu()
            y = torch.index_select(self.weight, 0, indices_cpu)
