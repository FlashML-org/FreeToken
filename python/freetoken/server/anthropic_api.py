...
async def anthropic_event_stream(
    ...
    def _open_thinking() -> str:
        ...
        return _event(AnthropicStreamEvent(
            type="content_block_start", index=block_index,
            # The SDK validates the opening block before any signature_delta.
            # Local reasoning is unsigned, just like the full-response path, but
            # the required string field must be present from the first event.
