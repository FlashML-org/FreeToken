...
@triton.jit
def moe_weighted_sum_reduce_kernel(
    input_ptr,
    input_stride_0,
    input_stride_1,
    input_stride_2,
    ...
    """Apply route weights and reduce in one fixed-order kernel."""
    token_block_id = tl.program_id(0)
    dim_block_id = tl.program_id(1)
    token_start = token_block_id * BLOCK_M
    token_end = min((token_block_id + 1) * BLOCK_M, token_num)
    ...
    for token_index in range(token_start, token_end):
        accumulator = tl.zeros((BLOCK_DIM,), dtype=tl.float32)
        input_t_ptr = input_ptr + token_index * input_stride_0 + offs_dim
        for route in tl.range(0, topk_num, num_stages=NUM_STAGE):
            value = tl.load(
                input_t_ptr + route * input_stride_1,
                mask=offs_dim < dim_end,
                other=0.0,
            )
            weight = tl.load(weights_ptr + token_index * weights_stride_0 + route * weights_stride_1)
