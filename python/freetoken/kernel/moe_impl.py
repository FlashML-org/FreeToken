...
def moe_weighted_sum_reduce_triton(
    ...
    """Graph-safe route weighting plus deterministic reduction in one launch."""
    import triton

    from .triton.fused_moe import moe_weighted_sum_reduce_kernel

    assert input.is_contiguous() and output.is_contiguous()
    assert weights.ndim == 2 and weights.stride(1) == 1
    token_num, topk_num, hidden_dim = input.shape
    ...
    moe_weighted_sum_reduce_kernel[grid](
        ...
        BLOCK_M=block_m, BLOCK_DIM=block_dim, NUM_STAGE=4,
    )
