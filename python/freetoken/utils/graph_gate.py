...
and keyed by runtime, target, toolchain, and policy identity so stale results do
not survive environment changes.
"""
...
def graph_capture_env() -> dict[str, str]:
    ...
    """
    try:
        if os.environ.get("FREETOKEN_ROCM_GRAPH_CAPTURE", "").strip().lower() in {
            "0",
            "false",
            "off",
            "no",
            ...
            return {}
        # CUDA must not pay for or inherit ROCm graph/BLAS probing.
        if not _is_rocm():
            return {}
        requested = _rocm_blas_request()
...
def _cache_identity() -> dict[str, object]:
    identity: dict[str, object] = {
        "device_kind": _device_kind(),
        "device": _device_name(),
        "rocm_blas": os.environ.get("FREETOKEN_ROCM_BLAS", "auto"),
        "blas_preference": os.environ.get("TORCH_BLAS_PREFER_HIPBLASLT", ""),
        "rocm_arch": os.environ.get("FREETOKEN_ROCM_ARCH", ""),
        "pytorch_rocm_arch": os.environ.get("PYTORCH_ROCM_ARCH", ""),
        "kernel_cache_gfx": os.environ.get("FREETOKEN_KERNEL_CACHE_GFX", ""),
    }
    try:
        import torch

        identity["torch"] = str(torch.__version__)
        identity["hip"] = str(torch.version.hip or "")
        if torch.cuda.is_available():
            props = torch.cuda.get_device_properties(torch.cuda.current_device())
            identity["gcn_arch"] = str(getattr(props, "gcnArchName", ""))
        else:
            identity["gcn_arch"] = ""
    except Exception:
        identity["torch"] = None
        identity["hip"] = None
        identity["gcn_arch"] = None
    return identity
...
def _load_cached() -> dict | None:
    try:
        ...
        if (
            ...
            and data.get("cache_identity") == _cache_identity()
...
def run_graph_gate() -> dict:
    ...
    result["cache_identity"] = _cache_identity()
    try:
