"""
DeepSeek-V4.1 Text and Vision Encoding (upstream MIT attribution: NOTICE).
...
def encode_messages(
    ...
    for message in normalized:
        if message.get("role") == "user" and isinstance(message.get("content"), list):
            message.setdefault("content_blocks", message.pop("content"))
    ordered = merge_tool_messages(normalized)
    ordered = sort_tool_results_by_call_order(copy.deepcopy(context) + ordered)[len(context):]
    # Sort tool results before collecting images so payload order matches the rendered spans.
    processed_messages, images = process_image_messages(ordered)
