...
def validate_replay_record(value: object) -> list[str]:
    ...
    problems: list[str] = []
    replay_schema = replay.get("schema") if isinstance(replay, dict) else None
    if replay_schema is None:
        replay_schema = value.get("schema")
    if replay_schema not in REPLAY_SCHEMAS:
    ...
    if replay_schema == "freetoken-replay-manifest-v2":
    ...
    elif replay_schema == "freetoken-replay-manifest-v1":
        ...
        if not isinstance(replay, dict) or not replay.get("route_digest"):
            problems.append("replay route digest is missing")
